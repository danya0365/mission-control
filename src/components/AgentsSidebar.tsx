...
import { ChevronRight, Loader2, Plus, Zap, ZapOff } from 'lucide-react';
...
export function AgentsSidebar({ workspaceId }: AgentsSidebarProps) {
  ...
  return (
    <aside className="w-full md:w-64 bg-mc-bg-secondary border-b md:border-b-0 md:border-r border-mc-border flex flex-col h-full">
      {/* Header */}
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <button 
              ...
              className="md:hidden p-1 hover:bg-mc-bg-tertiary rounded"
            >
              <ChevronRight className={`w-4 h-4 text-mc-text-secondary transition-transform ${!isMobileCollapsed ? 'rotate-90' : ''}`} />
            </button>
            <ChevronRight className="hidden md:block w-4 h-4 text-mc-text-secondary" />
      ...
      {/* Collapsible Content */}
      <div className={`flex flex-col flex-1 overflow-hidden ${isMobileCollapsed ? 'hidden md:flex' : 'flex'}`}>
        {/* Active Sub-Agents Counter */}
        {activeSubAgents > 0 && (
          <div className="mx-3 mb-3 px-3 py-2 bg-green-500/10 border border-green-500/20 rounded-lg">
